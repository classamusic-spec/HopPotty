...
///
/// ## The scene is `Art/pond/pond-stage.svg`
///
/// The place itself is the exported drawing, not a SwiftUI impression of it:
/// the same composition, at the same ``PondGeometry/referenceAspect``, with the
/// same band lines, that `Scripts/screens/pond.js` puts under the same forty-one
/// anchors. ``PondStageArtLayer`` hangs it in ``PondGeometry/stage(in:)`` and
/// continues the sky above and the meadow below, so a tall phone reads as more
/// air and more grass rather than as a stretched pond.
///
/// The three canvases below are what this view drew before the export pipeline
/// existed, and they are still what it draws when the asset is not in the
/// bundle — the catalog is generated and git-ignored, so an un-bootstrapped
/// checkout has no drawings at all:
///
...
/// Over the drawing exactly one thing moves — the shimmer, a broad band of sun
/// crossing the water — because everything else the drift canvas animated is now
/// *in* the picture, and a moving copy laid over a still original is a duplicate
/// rather than ambient motion. On the drawn fallback the whole drift layer runs:
/// the clouds take forty-one seconds to travel sixteen points, and the fish and
/// the dragonfly *shuttle* — a saturated sine holds them at one end of a short
/// trip for most of the cycle and moves them across quickly, so the eye catches
/// a fish crossing occasionally rather than a fish endlessly swimming. Either
/// way no two moving things share a period or a phase, so the scene never pulses
/// as one.
///
...
struct PondSceneView: View {
    ...
    var body: some View {
        GeometryReader { proxy in
            let size = proxy.size
            // `pond-stage.svg` is the composition below, drawn rather than
            // approximated. When it is not in the bundle the three canvases are
            // still there and still right — see `PondStageArtLayer`.
            let hasStageArt = HopArtwork.hasAsset(for: .pondStage)
            ZStack {
                if hasStageArt {
                    PondStageArtLayer()
                } else {
                ...
                // Only the drawn pond needs its foreground put back on top: the
                // stage art carries its own `pond-foreground` band inside the
                // picture, under the decorations — which is exactly where
                // `Scripts/screens/pond.js` composites them, drawing the whole
                // stage first and every unlocked item over it.
                if !hasStageArt {
                    PondForegroundLayer()
                }
...
// MARK: - The stage, drawn

/// `Art/pond/pond-stage.svg` — the composition itself, not an impression of it.
///
/// The file is the pond at ``PondGeometry/referenceAspect`` with the same band
/// lines this view's geometry names, generated by `Scripts/scene-art.js` from
/// the same numbers: horizon 0.30, water centred `(0.50, 0.62)` with radii
/// `0.39w × 0.23h`, foreground 0.93. So the drawing drops straight into
/// ``PondGeometry/stage(in:)`` and every `PondCatalog` anchor lands where it was
/// authored — a duckling in the water, a reed on the shore — with no second
/// opinion about where the waterline is.
///
/// `Scripts/screens/pond.js` composes the screen the same way: the stage first,
/// then every unlocked decoration over it, with Hop between the `decoration` and
/// `foreground` layers. Which means the drawing's own near grass sits *under*
/// the child's things here, and `PondForegroundLayer` is not drawn again on top.
///
/// ## Why the canvases are still in this file
///
/// The catalog is generated (`Scripts/build-assets.sh`) and git-ignored, so a
/// checkout that has not been bootstrapped has no drawings. A pond built from
/// paths is a pond; ``HopArtwork``'s placeholder blob behind a child's whole
/// collection is not. `PondSceneView` picks between them once, at the top.
///
/// ## What still moves
///
/// One thing: `pond-shimmer`, the broad low band of sun crossing the water. Every
/// other layer the drift canvas animated — the clouds, the fish, the pads, the
/// reeds, the bank flowers, the dragonfly — is *in* the drawing now, and drawing
/// a moving copy over a still original is a duplicate, not ambient motion. Light
/// is the exception because it belongs to no object.
private struct PondStageArtLayer: View {
    @Environment(\.hopTheme) private var theme
        ...
        return GeometryReader { proxy in
            ...
            ZStack(alignment: .topLeading) {
                Canvas(rendersAsynchronously: false) { context, canvasSize in
                    PondStageArtLayer.drawSurround(into: &context, size: canvasSize, ink: ink)
                }

                HopArtwork(.pondStage)
                    .frame(width: stage.width, height: stage.height)
                    .offset(x: stage.minX, y: stage.minY)

                PondStageSurfaceLight(stage: stage, sheen: ink.sheen)
            }
            .frame(width: max(1, size.width), height: max(1, size.height))
            .clipped()
        }
        // Dusk sits here rather than over the whole scene for the same reason
        // `PondStillLayer` put it here: the evening passes over the *place*, and
        // a child's own decorations are not dimmed by the time of day.
        .overlay { if let dusk = ink.dusk { dusk.allowsHitTesting(false) } }
        .allowsHitTesting(false)
        .accessibilityHidden(true)
    }

    /// The sky above the stage and the meadow below it.
    ///
    /// The frame is usually taller than the drawing, and the extra height has to
    /// read as more sky and more grass rather than as a stretched pond. Both
    /// strips are the colours the drawing's own edges already are: the sky
    /// continues in the stage's first gradient stop, and the ground below gets
    /// the meadow *and* the near-grass ink over it — the same two fills, in the
    /// same order, that `pond-foreground` lays down inside the picture, so the
    /// seam at the stage's bottom edge has nothing to show.
    static func drawSurround(into outer: inout GraphicsContext, size: CGSize, ink: PondPalette) {
        let stage = PondGeometry.stage(in: size)
        let width = max(1, size.width)

        let above = CGRect(x: 0, y: 0, width: width, height: max(0, stage.minY) + 1)
        outer.fill(
            Path(above),
            with: .linearGradient(
        ...
        outer.fill(Path(below), with: .color(ink.grassNear.opacity(0.34)))
    }
}

/// `pond-shimmer` over the stage art: a broad, very low band of light sliding
/// across the water, clipped to the water body the drawing and this view agree
/// on. Under a tenth of the water's own contrast, and it stops with the clock —
/// Reduce Motion, off screen, or a scene phase that is not `.active`.
private struct PondStageSurfaceLight: View {
    let stage: CGRect
    let sheen: Color

    var body: some View {
        HopPondTimeline { clock in
            Canvas(rendersAsynchronously: false) { outer, _ in
                var context = outer
                context.translateBy(x: stage.minX, y: stage.minY)

                let water = PondGeometry.water(width: stage.width, height: stage.height)
                ...
                let travel = CGFloat(clock.wave(period: HopMotion.pondShimmerPeriod))
                    * HopPondMotion.shimmerTravel * stage.width
