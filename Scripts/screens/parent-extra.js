...
function sheetOver(appearance, presenter, sheetInner, { top, radius = 12 } = {}) {
  ...
  return `<div style="position:relative;width:${W}px;height:${H}px;overflow:hidden;background:${P.midnight}">
    <div style="position:absolute;left:0;top:10px;width:${W}px;height:${H}px;overflow:hidden;border-radius:14px;
      ...
      ${presenter}
      <div style="position:absolute;inset:0;background:${alpha(P.midnight, .18)}"></div>
    </div>
  ...
  return phone(appearance, `
    <div class="fit" style="flex:1;display:flex;flex-direction:column;padding:0 24px 8px;overflow:hidden">
      ...
      <div style="flex:0 0 auto;margin-top:18px;display:flex;gap:11px;align-items:flex-start;
        ...
        ${iconTile(dark ? alpha('#FFFFFF', .08) : '#FFFFFF', MARK.hand(col.textTertiary, 17), { size: 30, radius: 9 })}
...
function childProfile(appearance = 'light') {
  ...
  const band = (title, sub, on) => `
    <div style="display:flex;align-items:center;gap:12px;min-height:52px;padding:9px 15px;
          ...
          ${AVATARS.map(([name, fill, ring], i) => `
            <div style="flex:1;display:flex;flex-direction:column;align-items:center;gap:6px">
              <div style="position:relative;padding:3px;border-radius:29px;
                ...
                ${avatarDisc(46, { fill, ring, ringWidth: 2 })}
              </div>
            </div>`).join('')}
        </div>
      </div>
  ...
  return phone(appearance, `
    <div class="fit" style="flex:1;display:flex;flex-direction:column;padding:0 24px 8px;overflow:hidden">
      ...
      <div style="flex:0 0 auto;position:relative;display:flex;justify-content:center">
        <div style="position:absolute;left:50%;top:24px;width:236px;height:236px;margin-left:-118px;border-radius:118px;
          background:${dark ? alpha(P.hopGreen, .12) : mix(P.hopGreenSoft, P.cloud, .28)}"></div>
        <div style="position:relative">${svg('Art/character/hop-cheer.svg', { width: 250 })}</div>
        ${sparkle(6, 40, 22, .95)}${sparkle(300, 22, 16, .8)}${sparkle(30, 176, 13, .7)}${sparkle(312, 150, 20, .85)}
      </div>
      ...
      <div style="flex:0 0 auto;margin-top:11px;display:flex;gap:11px;align-items:flex-start;padding:0 4px">
...
function settingsHub(appearance = 'light') {
  ...
  <div style="display:flex;flex-direction:column;height:${H}px;background:${col.backgroundSecondary}">
    ...
    ${navBar(col, 'Settings', { back: false, large: true })}
    <div class="fit" style="flex:1;display:flex;flex-direction:column;gap:10px;padding:2px ${PAGE}px 6px;overflow:hidden">
      ...
      ${listGroup(col, appearance, {
        ...
        rows: [
          ...
          listRow(col, {
            icon: tile(soft(P.hopGreen), MARK.check(col.brandAction, 16)),
...
function childProfiles(appearance = 'light') {
  ...
  const childRow = (name, detail, stars, disc, { active = false, last = false } = {}) => `
    <div style="display:flex;align-items:center;gap:12px;min-height:66px;padding:11px 14px;
      ${last ? '' : `box-shadow:inset 0 -1px 0 ${col.divider};`}">
      ${avatarDisc(42, disc)}
      <div style="flex:1;min-width:0">
        <div style="display:flex;align-items:center;gap:8px">
          <span style="${type('parentBody', { color: col.textPrimary, weight: 'semibold' })};font-size:16.5px">${name}</span>
        ...
        <div style="${type('parentCaption', { color: col.textTertiary })};font-size:12.5px;margin-top:2px">${detail}</div>
      </div>
      <div style="display:flex;align-items:center;gap:4px;flex:0 0 auto">
        ${MARK.star(col.celebration, 15)}
        <span style="${type('parentFootnote', { color: col.textSecondary, weight: 'semibold' })};font-size:13px;font-variant-numeric:tabular-nums">${stars}</span>
      </div>
      <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="${col.textTertiary}" stroke-width="2.8"
  ...
  <div style="display:flex;flex-direction:column;height:${H}px;background:${col.backgroundSecondary}">
    ...
    <div class="fit" style="flex:1;display:flex;flex-direction:column;gap:12px;padding:4px ${PAGE}px 6px;overflow:hidden">
      ...
      <div style="flex:0 0 auto;background:${col.surface};border-radius:${T.radius.l}px;overflow:hidden;
        box-shadow:${elevation(appearance, 'resting')}">
        ${childRow('Maya', 'Guided routine · every 45 minutes', '13', { fill: P.hopGreenSoft, ring: P.hopGreenLight }, { active: true })}
        ${childRow('Sam', 'Gentle · every 60 minutes', '4', { fill: P.pondBlueSoft, ring: P.pondBlue }, { last: true })}
      </div>

      ${listGroup(col, appearance, {
        rows: [
          listRow(col, {
            icon: iconTile(dark ? alpha(P.hopGreen, .24) : P.hopGreenSoft, MARK.check(col.brandAction, 16), { size: 29, radius: 8 }),
            label: `<span style="color:${col.brandAction}">Add a child</span>`, accessory: '', last: true,
          }),
        ],
        footer: 'Every child gets their own pond, stars and schedule. Nothing is shared between them.',
      })}
      ...
      <div style="flex:0 0 auto;background:${col.surface};border-radius:${T.radius.l}px;padding:14px 15px;
        ...
        <div style="flex:1">
          ...
          <div style="${type('parentCaption', { color: col.textSecondary })};font-size:13px;margin-top:3px;line-height:1.42">
            Give each child their own pond, stars and schedule.</div>
        </div>
      </div>
  ...
  <div style="display:flex;flex-direction:column;height:${H}px;background:${col.backgroundPrimary}">
    ...
    <div class="fit" style="flex:1;display:flex;flex-direction:column;padding:0 ${PAGE}px 6px;overflow:hidden">
      ...
      <div style="flex:0 0 auto;display:flex;align-items:center;gap:14px;background:${dark ? alpha(P.hopGreen, .13) : P.hopGreenSoft};
        border-radius:${T.radius.xl}px;padding:14px 16px">
        ${avatarDisc(48, { fill: '#FFFFFF', ring: P.hopGreenLight, ringWidth: 2 })}
        <div style="flex:1">
      ...
      <div style="flex:0 0 auto;display:flex;flex-direction:column;gap:14px;padding-top:18px">
      ...
      <div style="flex:0 0 auto">
        ...
        <div style="margin-top:10px">${secondaryButton(col, 'Restore purchase')}</div>
...
function parentGate(appearance = 'light') {
  ...
  const sheet = `
    ...
    <div class="fit" style="flex:1;display:flex;flex-direction:column;padding:0 22px 0;overflow:hidden">
      ...
      <div style="flex:0 0 auto;display:flex;align-items:center;gap:14px;margin-top:20px;
  ...
  return sheetOver(appearance, settingsHub(appearance), sheet, { top: 62 });
}
...
function progressEmpty(appearance = 'light') {
  ...
  const zeroRow = (glyph, tint, soft, text, last) => `
    ...
    <div class="fit" style="flex:1;display:flex;flex-direction:column;gap:9px;padding:0 ${PAGE}px 8px;overflow:hidden">
      ...
      <div style="flex:0 0 auto;background:${col.surface};border-radius:${T.radius.xl}px;padding:14px 16px 13px;
        ...
        <div style="margin-top:4px">
          ${zeroRow(MARK.ring, TINT.tried.tint, TINT.tried.soft, 'No potty visits logged yet')}
          ${zeroRow(MARK.star, TINT.star.tint, TINT.star.soft, 'None yet today', true)}
        </div>
      </div>
...
function quickReminderSheet(appearance = 'light') {
  ...
  const sheet = `
    <div class="fit" style="flex:1;display:flex;flex-direction:column;padding:2px 20px 0;overflow:hidden">
      ...
      <div style="flex:0 0 auto;display:flex;gap:11px;align-items:flex-start;margin-top:12px;padding:0 4px">
...
function bandLabel(text, { dark = false } = {}) {
  const ink = dark ? '#FFFFFF' : P.midnight;
  return `<div style="position:absolute;left:14px;top:12px;z-index:3;height:24px;padding:0 11px;border-radius:12px;
    ...
    <div style="position:absolute;left:0;top:0;width:${W}px;height:${BAND_A}px;overflow:hidden">
      ...
      <div style="position:relative;display:flex;flex-direction:column;height:100%">
          ...
          <div style="display:flex;gap:16px;width:100%">
            ...
            <div style="flex:1;display:flex;flex-direction:column;justify-content:space-between;padding:4px 0">
              <div style="display:flex;justify-content:space-between">
                ${appIcon(60, alpha('#FFFFFF', .5))}${appIcon(60, alpha('#FFFFFF', .5))}
              </div>
              <div style="display:flex;justify-content:space-between">
                ${appIcon(60, alpha('#FFFFFF', .5))}${appIcon(60, alpha('#FFFFFF', .5))}
              </div>
        ...
        <div style="flex:0 0 auto;margin:0 ${M - 8}px 12px;height:78px;border-radius:${T.radius.xl}px;
          ...
          ${appIcon(58, alpha('#FFFFFF', .62))}${appIcon(58, alpha('#FFFFFF', .62))}
          ${appIcon(58, alpha('#FFFFFF', .62))}${appIcon(58, alpha('#FFFFFF', .62))}
        </div>
      </div>
    </div>
...
function liveActivity(appearance = 'light') {
  ...
  const stepChip = (label, glyph, on) => `
    <div style="display:flex;flex-direction:column;align-items:center;gap:5px;flex:1">
      <div style="width:34px;height:34px;border-radius:17px;display:grid;place-items:center;
        background:${on ? P.hopGreenDeep : alpha('#FFFFFF', .12)}">${glyph(on ? '#FFFFFF' : alpha('#FFFFFF', .55), 17)}</div>
      <span style="${type('parentFootnote', { color: on ? '#FFFFFF' : alpha('#FFFFFF', .55), weight: on ? 'semibold' : 'medium' })};font-size:10px">${label}</span>
    </div>`;

  const wipe = (f, s = 20) => `<svg viewBox="0 0 24 24" width="${s}" height="${s}" fill="none" stroke="${f}" stroke-width="2.1" stroke-linecap="round" stroke-linejoin="round"><rect x="4.4" y="4.4" width="10" height="15.2" rx="5"/><path d="M14.4 19.6h5.2V9.2a4.8 4.8 0 0 0-5.2-4.8"/></svg>`;
  const flush = (f, s = 20) => `<svg viewBox="0 0 24 24" width="${s}" height="${s}" fill="none" stroke="${f}" stroke-width="2.1" stroke-linecap="round" stroke-linejoin="round"><path d="M19.4 12a7.4 7.4 0 1 1-2.2-5.2"/><path d="M19.8 3.6v5h-5"/></svg>`;
  ...
  const activityCard = `
    <div style="width:${W - M * 2}px;border-radius:${T.radius.xl}px;background:${alpha('#FFFFFF', .16)};
      ...
      <div style="display:flex;align-items:center;gap:11px">
        ...
        <div style="width:26px;height:26px;border-radius:13px;background:${alpha('#FFFFFF', .18)};display:grid;place-items:center;flex:0 0 auto">
          ${MARK.pause('#FFFFFF', 13)}
        </div>
      </div>
  ...
  <div style="position:relative;width:${W}px;height:${H}px;overflow:hidden;background:${P.midnight}">
    ...
    <div style="position:absolute;left:0;top:0;width:${W}px;height:${BAND_A}px;overflow:hidden;
      ...
      <div style="position:relative;display:flex;flex-direction:column;height:100%;align-items:center">
        <div style="height:44px;flex:0 0 auto"></div>
        ${island}
      </div>
    </div>
    ...
    <div style="position:absolute;left:0;top:${BAND_A + 10}px;width:${W}px;height:${BAND_B}px;overflow:hidden">
      ...
      <div style="position:relative;display:flex;flex-direction:column;height:100%;align-items:center;padding:0 ${M}px">
        ...
        ${activityCard}
        <div style="${type('parentCaption', { color: alpha('#FFFFFF', .6) })};font-size:12px;text-align:center;margin-top:14px">
          No countdown on purpose. A visible clock reads as pressure to a small child.</div>
        <div style="flex:1"></div>
        ${homeIndicator('#FFFFFF')}
      </div>
    </div>
...
function insightsPad(appearance = 'light') {
  ...
  const gutter = 28, gap = 20;
  const colW = Math.round((w - gutter * 2 - gap) / 2);

  const cardBox = (inner, { pad = '16px 18px 14px' } = {}) => `
  ...
  const dayShape = (values, peak) => `<div style="display:flex;align-items:flex-end;gap:5px;height:52px">
    ${values.map((v, i) => `<div style="flex:1;height:${Math.max(9, v * 52)}px;border-radius:3px;
            ...
            ${cardBox(`
              ...
              <div style="margin:8px -4px 0">
                ${sparkline([0.42, 0.5, 0.44, 0.58, 0.55, 0.63, 0.67], {
                  w: colW - 28, h: 96, stroke: TINT.check.tint, fill: TINT.check.tint,
                })}
              </div>
